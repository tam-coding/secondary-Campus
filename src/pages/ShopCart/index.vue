<template>
  <div class="cart">
    ...
    <div class="cart-main">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cartInfo,index) in cartInfoList" :key="index">
          ...
          <li class="cart-list-con5">
            <button  class="mins" @click="handle('minus',-1,cartInfo)" :disabled="cartInfo.skuNum==1">-</button>
            <input autocomplete="off" type="text" :value="cartInfo.skuNum" minnum="1" class="itxt"  @change="handle('change',$event.target.value*1,cartInfo) " >
            <a  class="plus"  @click="handle('plus',1,cartInfo)">+</a>
          </li>
          <li class="cart-list-con6">
            <span class="sum">{{cartInfo.skuPrice*cartInfo.skuNum}}</span>
          </li>
          <li class="cart-list-con7">
            <a  class="sindelet" @click="deleteById(cartInfo)" style="cursor:pointer">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
import { mapGetters } from 'vuex'
  export default {
    ...
    methods: {
      ...
      //type：区分操作 disNUm 变化的量 carInfo获取商品id
      handle:throttle( async function(type,disNum,cartInfo){
        switch (type) {
          ...
          case "plus":
            ...
            break;
          case "change":
            let PInteger=/^[1-9]\d*$/g
            if(!PInteger.test(disNum)){
              disNum=0
            }else{
              disNum=disNum*1-cartInfo.skuNum
